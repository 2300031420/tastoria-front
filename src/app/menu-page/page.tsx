...
export default function MenuPage() {
  ...
  useEffect(() => {
    const fetchMenu = async () => {
      try {
        setIsLoading(true);
        // Get cafeName from URL query params, default to 'tastoria'
        const urlParams = new URLSearchParams(window.location.search);
        const cafeName = urlParams.get('cafe') || 'tastoria';
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}api/menu/all?cafeName=${cafeName}`);
  ...
  useEffect(() => {
    ...
    // Get cafeName from URL query params
    const urlParams = new URLSearchParams(window.location.search);
    const cafeName = urlParams.get('cafe') || 'tastoria';

    socket.on("menuStatusChanged", ({ itemId, paused, cafeName: itemCafeName }) => {
      // Only update if the item belongs to the current cafe
      if (itemCafeName && itemCafeName !== cafeName) {
        return;
      }
